...
function offline(reason) {
  return [
    "### ⚠️ Analisis Offline MONA LISA",
    "",
    "Sistem gagal memperoleh respons dari 9Router.",
    "",
    "**Penyebab teknis:**",
    String(reason || "AI tidak tersedia"),
    "",
    "**Rekomendasi:**",
...
app.post("/api/ai-insight", async function (req, res) {
  try {
    const body = req.body || {};
    ...
    if (!response.ok) {
      ...
      return res.status(502).json({
        ...
        error: result && result.error ? result.error.message : text,
        insight: offline(result && result.error ? result.error.message : text)
      });
    }

    const insight = result && result.choices && result.choices[0] && result.choices[0].message
